...
def is_report(m):
    ...
    # '보고서. 원문'처럼 구두점 오타가 낀 표기도 인정 (9/11 이다연 SKT 인뎁스 미수집 사고)
    ...
    # 헤더는 '[통신서비스(Overweight) | 다올 통신·음식료 이다연]'처럼 다올이 대괄호 중간에 와도 인정.
    analyst_hdr=re.search(r'\[[^\]]{0,80}다올[^\]]*\]|담당\s*[가-힣]{2,4}',t[:200])
